      ...
      <div className="border-t pt-3">
        ...
        {form.prestaciones.map((p, i) => (
          <div key={i} className="flex gap-2 mb-2">
            <input className={`${inputCls} flex-1`} placeholder={`Prestación ${i + 1}${i === 0 ? ' *' : ''}`}
              value={p.glosa} onChange={e => setPrest(i, 'glosa', e.target.value)} />
            <input className={`${inputCls} w-28`} placeholder={`Valor ${i + 1}`}
              value={p.valor} onChange={e => setPrest(i, 'valor', e.target.value)} />
          </div>
        ))}
      </div>
